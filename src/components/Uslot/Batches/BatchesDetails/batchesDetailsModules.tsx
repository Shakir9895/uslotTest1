...
import BatchesStudentTableRow from './product-table-row';
import BatchesNotificationTableRow from './batchesNotificanTableRow';
import BatchesModuleTableRow from './batchesModuleTableRow';
...
const TABLE_HEAD = [
  ...
  { id: 'syllabus', label: 'Syllabus', width: 600 },
...
export default function BatchesDetailsModules() {
  ...
  const [dialogTitle, setDialogTitle] = useState('Add Session')
  ...
  //NewSessionDialog
  const NewSessionDialog = useCallback(
    (value?: string,id?:string) => {
      setNewSession(true);
      if(value ==='NewSession'){
        setDialogTitle('Add Session')

      }else if(value ==='EditSession'){
  ...
  return (
    <>
          ...
          <CustomBreadcrumbs
            ...
            action={
              <Button
                ...
                onClick={() => NewSessionDialog('NewSession')}
                ...
                <TableBody>
                  {dataFiltered
                    ...
                    .map((row) => (
                      ...
                      <BatchesModuleTableRow
                        key={row.id}
                        row={row}
                        selected={table.selected.includes(row.id || "")}
                        onSelectRow={() => table.onSelectRow(row.id)}
                        onDeleteRow={() => handleDeleteRow(row.id)}
                        onEditRow={() => NewSessionDialog('EditSession',row.id)}
                      />
                    ))}
      ...
      >
        ...
        <NewSessionForm dialogTitle={dialogTitle} onClick={onCloseForm} />
      </Dialog>
