...
import CourseCategoryTableRow from './user-managment-table-row';
import DialogTitle from '@mui/material/DialogTitle';
import { useTheme } from '@mui/material/styles';
import AddCategoryForm from './addcategoryform';
import { useGetEvents } from 'src/api/calendar';
import { DeleteCategoryApiCall, GetCourseCategoryApiCall, SingleViewCategoryApiCall } from 'src/api/uslot/academics';
import { useSelector } from 'react-redux';
import { RootState } from 'src/redux/store';
import { useDispatch } from 'react-redux';
import { SingleViewCategory } from 'src/redux/slices/app';
...
const TABLE_HEAD = [
  { id: 'no', label: 'No' },
  { id: 'name', label: 'Picture', width: 200 },
  { id: 'category Name', label: 'Category Name', width: 380 },
...
export default function CourseCategoryListView() {
  ...
  const handleDeleteRow = useCallback(
    (id?: string) => {
      DeleteCategoryApiCall(id,modifyFun,enqueueSnackbar);
    },
    [CategoryList]
  );
  ...
  const NewCategoryDialog = (value?: string, id?: string) => {
    setNewCategory(true);
    console.log(value, id)
    if (value === "newcategory") {
      setDialogTitle('Add Category');
      dispatch(SingleViewCategory({ data: "" }))
    }
    else if (value === 'editcategory') {
      setDialogTitle('Edit Category');


      //SingleViewCategoryApiCall
      SingleViewCategoryApiCall(id);
  ...
  return (
    <>
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              onClick={() => NewCategoryDialog("newcategory")}
              variant="contained"
                  ...
                  {dataFiltered
                    ...
                    .map((row, index) => (
                      <CourseCategoryTableRow
                        ...
                        onEditRow={() => NewCategoryDialog("editcategory", row.id)}
                      />
                    ))}
      ...
      >
        ...
        <AddCategoryForm dialogTitle={dialogTitle} onClick={onCloseForm} setNewCategory={setNewCategory} />
