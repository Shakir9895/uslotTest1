...
import { CreateQuizbyCourseId, UpdateQuizByIdApiCall, getSingleQuizById } from 'src/api/uslot/academics';
...
export default function CreateQuizForm({ currentProduct, Heading }: Props) {
  ...
  const { SingleCourse, SingleQuiz } = useSelector((state: RootState) => state.app);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (Heading === 'Edit Quiz') {
        UpdateQuizByIdApiCall(id,body,enqueueSnackbar)

      } else {
        CreateQuizbyCourseId(course_id, body, enqueueSnackbar, reset)
      }
  ...
  //UseEffect
  useEffect(() => {
    setValue('question', SingleQuiz?.question)
    setValue('option_a', SingleQuiz ? SingleQuiz?.options[0] : "");
    setValue('option_b', SingleQuiz ? SingleQuiz?.options[1] : "");
    setValue('option_c', SingleQuiz ? SingleQuiz?.options[2] : "");
    setValue('option_d', SingleQuiz ? SingleQuiz?.options[3] : "");
    setSelectedOption(SingleQuiz?.correct_answer ? SingleQuiz?.correct_answer : 'optionA')
  }, [setValue, SingleQuiz]);
